...
void causalMasking(TensorView &S)
{
    auto s_shape = S.shape();

    for (size_t i = 0; i < s_shape[0]; ++i)
    {
        for (size_t j = i + 1; j < s_shape[1]; ++j)
        {
            S(i, j) = -1e9;
        }
    }
...
TensorView forwardAttention(TensorView &normX, TensorView &attnWeight, TensorView &attnBias, TensorView &attnProjWeight, TensorView &attnProjBias, ArenaAllocator &alloc)
{
